...
export function LikeButton(props: NumLikeProps) {
  ...
  async function onSubmit() {
    ...
    window.localStorage.setItem(`isLiked-${videoId}`, isLiked ? "false" : "true");
    window.localStorage.setItem(`currentLikes-${videoId}`, updatedLikes.toString());
  ...
  useEffect(() => {
    const storedIsLiked = window.localStorage.getItem(`isLiked-${videoId}`);
    const storedLikesRaw = window.localStorage.getItem(`currentLikes-${videoId}`);
